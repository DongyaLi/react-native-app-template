...
function catchError(error: any) {
  ...
  if (response.status === 401) {
    ...
    //   (window as any).SESSION_TIMEOUT_LOGOUT &&
    //   (window as any).SESSION_TIMEOUT_LOGOUT();
...
function fetchData(url: string, params: any) {
  ...
  const realUrl =
    url.indexOf('http') === -1 ? `${Config.API_SERVERS.BATMAN_BFF}${url}` : url;
...
export const fetchJSONStringByPost = fetchJSONByMethod('JSONPOST', {
  'Content-Type': 'application/json;charset=UTF-8',
});

export const fetchJSONStringByPut = fetchJSONByMethod('JSONPUT', {
  'Content-Type': 'application/json;charset=UTF-8',
});
